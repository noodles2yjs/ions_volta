...
func (l *LoginController) Post() {
	...
	o := orm.NewOrm()
	qs := o.QueryTable("sys_user").Filter("user_name", username).Filter("password", md5_pwd)
	is_exist := qs.Exist()
	...
	if !is_exist {
		...
		ret_map["msg"] = "验证码错误"
		l.Data["json"] = ret_map
	} else if uerInfo.IsActive == 0 {
		ret_map["code"] = 10001
		ret_map["msg"] = "用户已经停用,请联系管理员"
		l.Data["json"] = ret_map
	} else {

		ret := fmt.Sprintf("登录的post请求，登录成功，登录信息：username:%s;pwd:%s", username, md5_pwd)
		logs.Info(ret)
		l.SetSession("id", uerInfo.Id)
		ret_map["code"] = 200
		ret_map["msg"] = "登录成功"
		l.Data["json"] = ret_map
	}
